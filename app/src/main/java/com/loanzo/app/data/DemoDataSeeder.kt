...
class DemoDataSeeder @Inject constructor(
    ...
    companion object {
        ...
        const val DEMO_AGENT_ACCOUNT_ID = "demo_agent_abhisi"
        const val DEMO_AGENT_2_ACCOUNT_ID = "demo_agent_sunil"
        ...
        const val DEMO_MEERA_ID = "demo_meera_sen"
        const val DEMO_KUNAL_ID = "demo_kunal_rawat"
        const val DEMO_ALOK_ID = "demo_alok_trivedi"
    ...
    private suspend fun seedGlobalEntities(currentUserId: String?, now: Long, oneDayMs: Long) {
        val currentDbUser = if (!currentUserId.isNullOrBlank()) database.userDao().getUserById(currentUserId) else null
        val isCurrentUserAgent = currentDbUser?.role?.uppercase() == "AGENT"
        val isCurrentUserAdmin = currentDbUser?.role?.uppercase() == "ADMIN"

        val targetAgentId = (if (isCurrentUserAgent) currentUserId else null) ?: DEMO_AGENT_ACCOUNT_ID
        val targetAdminId = (if (isCurrentUserAdmin) currentUserId else null) ?: DEMO_ADMIN_ACCOUNT_ID
        val targetConsumerId = (if (!isCurrentUserAgent && !isCurrentUserAdmin) currentUserId else null) ?: DEMO_USER_ACCOUNT_ID
        val targetConsumerName = currentDbUser?.name?.takeIf { it.isNotBlank() } ?: "Satyam Kumar"
        val targetConsumerPhone = currentDbUser?.phone?.takeIf { it.isNotBlank() } ?: "+91 70615 59039"
        ...
        val demoUsers = listOf(
            ...
            // Dedicated Field Agent 2 Account (for Cross-Verification Pair)
            UserEntity(
                userId = DEMO_AGENT_2_ACCOUNT_ID,
                name = "Sunil Patil (Field Officer 2)",
                email = "sunil.patil@loanzo.app",
                phone = "+91 98233 88776",
                ...
                panVerified = true,
                bankVerified = true,
                bankAccountNumber = "4081928374619",
                bankIfsc = "ICIC0001092",
                panNumber = "CKLSP8831Q",
                aadhaarNumber = "6541 2398 7102",
                upiId = "sunil.patil@okicici",
                dateOfBirth = "22/07/1988",
                address = "Flat 502, Embassy Residency, Domlur, Bengaluru 560071",
                agentStatus = "APPROVED",
                isOnDuty = true,
                totalAgentEarnings = 5750.0
            ),
            ...
            // Amit Verma (MSME Retailer - PENDING KYC for Admin Review testing)
            UserEntity(
                userId = DEMO_AMIT_ID,
                name = "Amit Verma",
            ...
            // Sneha Roy (Diagnostic Clinic Partner - IN_REVIEW KYC for Admin Inspection)
            UserEntity(
            ...
            // Meera Sen (New Borrower - PENDING KYC)
            UserEntity(
                userId = DEMO_MEERA_ID,
                name = "Meera Sen",
                ...
                panVerified = false,
                bankVerified = false,
                bankAccountNumber = "9981245012341",
                bankIfsc = "ICIC0001044",
                panNumber = "NMPMS9921B",
                aadhaarNumber = "7890 1234 5678",
                upiId = "meera.sen@okicici",
                dateOfBirth = "14/02/1994",
                address = "Sector 62, Noida, UP 201309"
            ),
            // Kunal Rawat (Fleet Driver - RE_KYC_REQUESTED)
            UserEntity(
                userId = DEMO_KUNAL_ID,
                name = "Kunal Rawat",
                ...
                password = "password123",
                role = "BORROWER",
                kycStatus = "RE_KYC_REQUESTED",
                aadhaarVerified = true,
                panVerified = false,
                bankVerified = true,
                bankAccountNumber = "3319028471928",
                bankIfsc = "SBIN0008811",
                panNumber = "PMPKR8832C",
                aadhaarNumber = "9123 4567 8901",
                upiId = "kunal.rawat@oksbi",
                dateOfBirth = "19/11/1990",
                address = "Andheri East, Mumbai, MH 400069"
            ),
            // Alok Trivedi (High Risk / Tax Mismatch - REJECTED)
            UserEntity(
                userId = DEMO_ALOK_ID,
                name = "Alok Trivedi",
                email = "alok.trivedi@demo.loanzo.app",
                phone = "+91 98119 55663",
                ...
                panVerified = false,
                bankVerified = false,
                bankAccountNumber = "1109283746592",
                bankIfsc = "PUNB0004912",
                panNumber = "QMPAT7741D",
                aadhaarNumber = "2345 6789 0192",
                upiId = "alok.trivedi@okpnb",
        ...
        demoUsers.forEach { user ->
            val existing = database.userDao().getUserById(user.userId)
                ?: if (user.username.isNotBlank()) database.userDao().getUserByUsername(user.username) else null
            val userToInsert = if (existing != null) {
                user.copy(
                    role = existing.role,
                    agentStatus = existing.agentStatus,
                    isOnDuty = existing.isOnDuty
                )
            } else {
                user
            }
            val cartoonAvatar = CartoonAvatarHelper.getCartoonAvatarUrl(userToInsert.name.ifBlank { userToInsert.userId })
            database.userDao().insertUser(userToInsert.copy(profilePhotoUri = userToInsert.profilePhotoUri.ifBlank { cartoonAvatar }))
        }
        ...
        val allDemoLoans = listOf(
            ...
            // Closed Loan: Rajesh Gupta -> Rahul Sharma (15K, Fully Repaid Ahead of Schedule)
            LoanEntity(
        ...
        // ==========================================
        val agentAppAbhisi = AgentApplicationEntity(
            applicationId = "app_agent_demo_${DEMO_AGENT_ACCOUNT_ID}",
            userId = DEMO_AGENT_ACCOUNT_ID,
            applicantName = "Abhisi",
            applicantPhone = "+91 98100 12345",
            applicantEmail = "abhisi@loanzo.app",
        ...
        val agentAppSunil = AgentApplicationEntity(
            applicationId = "app_agent_demo_${DEMO_AGENT_2_ACCOUNT_ID}",
            userId = DEMO_AGENT_2_ACCOUNT_ID,
            applicantName = "Sunil Patil",
            applicantPhone = "+91 98233 88776",
            applicantEmail = "sunil.patil@loanzo.app",
            experienceYears = "5+ Years",
            priorDomain = "Real Estate & Vehicle Appraiser",
            policeVerificationNumber = "PV/BLR/2022/44109",
            policeStation = "Domlur Police Station, Bengaluru",
            policeVerificationDate = "20/08/2023",
            permanentAddress = "Flat 502, Embassy Residency, Domlur, Bengaluru",
            operatingCity = "Bengaluru",
            operatingPincode = "560071",
            vehicleType = "Four-Wheeler",
            drivingLicenseNumber = "KA-0520090019284",
            status = "APPROVED",
            reviewedAt = now - (14 * oneDayMs)
        )
        val agentAppKavita = AgentApplicationEntity(
            applicationId = "app_agent_demo_kavita",
            userId = "demo_applicant_kavita",
            applicantName = "Kavita Reddy",
            applicantPhone = "+91 97400 99881",
            applicantEmail = "kavita.reddy@gmail.com",
            experienceYears = "1-2 Years",
            priorDomain = "Banking / NBFC",
            policeVerificationNumber = "PV/HYD/2024/77312",
            policeStation = "Madhapur Police Station",
            policeVerificationDate = "10/02/2026",
            permanentAddress = "Plot 12, Hitec City, Hyderabad",
            operatingCity = "Hyderabad",
            operatingPincode = "500081",
            vehicleType = "Two-Wheeler",
            drivingLicenseNumber = "TS-0920210088192",
            status = "PENDING",
            submittedAt = now - (1 * oneDayMs)
        )
        database.agentDao().insertApplication(agentAppAbhisi)
        database.agentDao().insertApplication(agentAppSunil)
        database.agentDao().insertApplication(agentAppKavita)

        val agentIdsToSeed = if (isCurrentUserAgent && currentUserId != null) {
            listOf(DEMO_AGENT_ACCOUNT_ID, currentUserId).distinct()
        } else {
            listOf(DEMO_AGENT_ACCOUNT_ID)
        }
        agentIdsToSeed.forEach { agentId ->
            val demoVisits = listOf(
                AgentVisitEntity(
                    ...
                    agentId = agentId,
                    assignedAgentName = "Abhisi",
                    agentPhone = "+91 98765 00111",
                    loanId = DEMO_LOAN_LENT_ID,
                ...
                AgentVisitEntity(
                    ...
                    agentId = agentId,
                    assignedAgentName = "Sunil Patil",
                    agentPhone = "+91 98450 99881",
                    loanId = DEMO_LOAN_PLATFORM_2,
                    visitType = "COLLATERAL_VERIFICATION",
        ...
        // --- Live Dual-Agent Swapped Verification Pair on Platform Loan 1 ---
        val dualCrossVisits = listOf(
            // Officer 1 (Abhisi): Verified Borrower's premise & physical machinery
            AgentVisitEntity(
                visitId = "visit_cross_demo_1",
                agentId = DEMO_AGENT_ACCOUNT_ID,
                assignedAgentName = "Abhisi (Officer 1)",
                agentPhone = "+91 98765 00111",
                loanId = DEMO_LOAN_PLATFORM_1,
                crossVerificationPairId = "cross_pair_demo_machinery",
                isCrossVerification = true,
                counterpartVisitId = "visit_cross_demo_2",
                verificationStage = "STAGE_2_SWAPPED",
                visitType = "CROSS_PREMISE_INSPECTION",
                title = "CNC Lathe Machine Physical Verification",
                ...
                appraisedValue = 150000.0,
                counterpartAppraisedValue = 148000.0,
                valuationDiscrepancyPercent = 1.33,
                ...
                status = "COMPLETED",
                agentRemarks = "Machinery physically inspected. Serial Jyoti-640 verified. Purity and electrical load tests passed. Business operational with 5 employees.",
                completedAt = now - (1 * oneDayMs),
                isBorrowerIdentityVerified = true,
                isCollateralAuthentic = true
            ),
            // Officer 2 (Sunil Patil): Swapped verification of Lender & independent appraisal
            AgentVisitEntity(
                visitId = "visit_cross_demo_2",
                agentId = DEMO_AGENT_2_ACCOUNT_ID,
                assignedAgentName = "Sunil Patil (Officer 2)",
                agentPhone = "+91 98450 99881",
                loanId = DEMO_LOAN_PLATFORM_1,
                crossVerificationPairId = "cross_pair_demo_machinery",
                isCrossVerification = true,
                counterpartVisitId = "visit_cross_demo_1",
                verificationStage = "STAGE_2_SWAPPED",
                visitType = "CROSS_COUNTERPARTY_INSPECTION",
                title = "CNC Lathe Machine Cross-Counterparty Audit",
                borrowerName = "Amit Verma",
                borrowerPhone = "+91 98111 22334",
                borrowerAddress = "Shop 14, Karol Bagh Market, New Delhi 110005",
                lenderName = "Vikram Malhotra",
                lenderPhone = "+91 98450 11223",
                lenderAddress = "Embassy Golf Links, Domlur, Bengaluru",
                targetAddress = "Embassy Golf Links, Domlur, Bengaluru",
                targetLatitude = 12.9490,
                targetLongitude = 77.6475,
                scheduledDate = "Yesterday",
                ...
                collateralEstimatedValue = 1850000.0,
                collateralPledgedValue = 150000.0,
                appraisedValue = 148000.0,
                counterpartAppraisedValue = 150000.0,
                valuationDiscrepancyPercent = 1.33,
                officerRecommendation = "RECOMMEND_APPROVAL",
                isCounterpartAnonymous = true,
                handshakePin = "6284",
                ...
                visitStageStatus = "COMPLETED",
                status = "COMPLETED",
                agentRemarks = "Lender counterparty verified in Domlur. Independent appraisal confirms industrial asset valuation within 1.3% variance of specs.",
                completedAt = now - (1 * oneDayMs),
                isBorrowerIdentityVerified = true,
                isCollateralAuthentic = true
            ),
            // Unassigned Backlog Visit for Admin Dispatch Testing
            AgentVisitEntity(
                visitId = "visit_demo_unassigned_dispatch",
                agentId = "UNASSIGNED",
                loanId = DEMO_LOAN_PLATFORM_3,
                visitType = "COLLATERAL_VERIFICATION",
                title = "Industrial Silent Genset Inspection",
                borrowerName = "Vikram Malhotra",
                borrowerPhone = "+91 98450 11223",
                borrowerAddress = "Plot 88, Electronic City Phase 1, Bengaluru 560100",
                lenderName = "Amit Verma",
                lenderPhone = "+91 98111 22334",
                lenderAddress = "Shop 14, Karol Bagh Market, New Delhi 110005",
                targetAddress = "Plot 88, Electronic City Phase 1, Bengaluru 560100",
                targetLatitude = 12.8452,
                targetLongitude = 77.6602,
                scheduledDate = "Today",
                scheduledTimeSlot = "04:00 PM - 05:30 PM",
                payoutAmount = 1400.0,
                collateralItemName = "Cummins 250kVA Silent Diesel Genset",
                collateralEstimatedValue = 1200000.0,
                collateralPledgedValue = 200000.0,
                status = "UNASSIGNED",
                visitStageStatus = "SCHEDULED",
                handshakePin = "5190",
                agentRemarks = "Pending officer allocation. Available for Single or Dual Swapped dispatch."
            )
        )
        database.agentDao().insertVisits(dualCrossVisits)
        ...
        // ==========================================
        // 11. SEED SUPPORT TICKETS (Member & Admin Views)
        // ==========================================
        val demoTickets = listOf(
            SupportTicketEntity(
                ...
                priority = "HIGH",
                subject = "Double deduction on EMI repayment via UPI Auto-Debit",
                description = "My bank account was debited twice for ₹8,834 on 25th of last month. UTR numbers: HDFC84920192 and HDFC84920193. Please verify and initiate refund.",
                status = "CALLBACK_SCHEDULED",
                relatedLoanId = DEMO_LOAN_PLATFORM_2,
                scheduledCallbackAt = now + (2 * 3600_000L),
                adminNotes = "Checked banking logs, identified double webhook trigger. Callback scheduled to confirm NEFT reversal details.",
                resolvedAt = null,
                resolutionNotes = null,
                feedbackRating = null,
            ...
            SupportTicketEntity(
                ...
                userId = targetConsumerId,
                userName = targetConsumerName,
                userPhone = targetConsumerPhone,
                category = "LOAN_ISSUE",
                priority = "MEDIUM",
                subject = "Tenure extension request for Education Loan",
                description = "Requesting a 3-month tenure extension due to delayed semester exams. Current EMI is ₹2,200/mo.",
                status = "OPEN",
                relatedLoanId = DEMO_LOAN_BORROWED_ID,
            ...
            SupportTicketEntity(
                ...
                category = "COLLATERAL",
                priority = "HIGH",
                subject = "Gold jewelry release procedure after full loan closure",
                description = "I have cleared my medical emergency loan completely. How do I collect the 24K gold necklace deposited in the central vault?",
                status = "RESOLVED",
                relatedLoanId = DEMO_LOAN_CLOSED_ID,
                scheduledCallbackAt = now - (2 * oneDayMs),
                adminNotes = "Called user and explained doorstep custody release protocol. NOC issued in vault.",
                resolvedAt = now - (1 * oneDayMs),
                resolutionNotes = "Explained doorstep physical handover by empaneled field agent Abhisi. Handover OTP verified, collateral safely received by borrower.",
                feedbackRating = 5,
                feedbackComment = "Prompt callback by the admin! Received my gold with 100% security seal intact.",
                createdAt = now - (3 * oneDayMs),
            ...
            SupportTicketEntity(
                ...
                category = "KYC_HELP",
                priority = "MEDIUM",
                subject = "DigiLocker Aadhaar XML verification error during re-KYC",
                description = "DigiLocker OTP was failing with error code DL-502. Resolved after clearing cache.",
                status = "RESOLVED",
                relatedLoanId = null,
                scheduledCallbackAt = now - (5 * oneDayMs),
                adminNotes = "UIDAI server maintenance window caused temporary timeout.",
                resolvedAt = now - (4 * oneDayMs),
                resolutionNotes = "Advised re-attempt after UIDAI sync. Verification completed successfully.",
                feedbackRating = 4,
                feedbackComment = "Very helpful support desk.",
                createdAt = now - (6 * oneDayMs),
                updatedAt = now - (4 * oneDayMs)
            ),
            SupportTicketEntity(
                ticketId = "tkt_demo_005",
                userId = DEMO_RAJESH_ID,
                userName = "Rajesh Gupta",
                userPhone = "+91 98765 00112",
                category = "PAYMENT_DISPUTE",
                priority = "URGENT",
                subject = "FY 2025-26 TDS certificate and interest statement download",
                description = "Need Form 16A / TDS certificate for tax filing for income received on peer lending notes.",
                status = "ESCALATED",
                relatedLoanId = DEMO_LOAN_PLATFORM_3,
                scheduledCallbackAt = null,
                adminNotes = "Escalated to finance accounting nodal officer for generation.",
                resolvedAt = null,
                resolutionNotes = null,
                feedbackRating = null,
                feedbackComment = null,
                createdAt = now - (12 * 3600_000L),
                updatedAt = now - (3 * 3600_000L)
            )
        )
        demoTickets.forEach { database.supportTicketDao().insertTicket(it) }

        // ==========================================
        val demoAdminRequests = listOf(
            AdminRequestEntity(
                requestId = "admin_req_demo_deepak",
                userId = "demo_staff_deepak",
                userName = "Deepak Verma",
                userPhone = "+91 97110 44556",
                userEmail = "deepak.verma@fintech.in",
                currentRole = "MEMBER",
                requestedRole = "ADMIN",
                reason = "Risk Management Officer joining platform operations. Need clearance to inspect collateral valuations, mediation hearings and dispatch field agents.",
                status = "PENDING",
                requestedAt = now - (2 * oneDayMs)
            ),
            AdminRequestEntity(
                requestId = "admin_req_demo_neha",
                userId = "demo_staff_neha",
                userName = "Neha Kapoor",
                userPhone = "+91 98201 33441",
                userEmail = "neha.kapoor@loanzo.legal",
                currentRole = "MEMBER",
                requestedRole = "ADMIN",
                reason = "Senior Legal Counsel joining risk compliance desk. Clearance required to review dispute arbitrations and inspect digital NOC agreements.",
                status = "PENDING",
                requestedAt = now - (1 * oneDayMs)
            )
        )
        demoAdminRequests.forEach { database.adminRequestDao().insertRequest(it) }
    }

    /**
     * Clears all demo entities cleanly from the database.
     * Restores current user's real state by purging fake demo KYC, Aadhaar, PAN and DigiLocker records.
     */
    suspend fun clearDemoData(currentUserId: String): Result<String> = withContext(Dispatchers.IO) {
        try {
            // 1. Reset current user's fake demo KYC / DigiLocker / Aadhaar / PAN values back to pristine unverified state
            val currentUser = database.userDao().getUserById(currentUserId)
            if (currentUser != null) {
                val isDemoAadhaar = currentUser.aadhaarNumber == "9876 5432 1098" || currentUser.aadhaarNumber == "4532 1098 7654"
                val isDemoPan = currentUser.panNumber == "ABCDE1234F" || currentUser.panNumber == "ADMKR7892L"
                val isDemoBank = currentUser.bankAccountNumber == "5010049281928"

                val resetAadhaar = if (isDemoAadhaar) "" else currentUser.aadhaarNumber
                val resetPan = if (isDemoPan) "" else currentUser.panNumber
                val resetBank = if (isDemoBank) "" else currentUser.bankAccountNumber
                val resetIfsc = if (isDemoBank) "" else currentUser.bankIfsc

                val hasRealKyc = resetAadhaar.isNotBlank() && resetPan.isNotBlank()
                val resetKycStatus = if (hasRealKyc) currentUser.kycStatus else "PENDING"
                val resetAadhaarVerified = resetAadhaar.isNotBlank() && currentUser.aadhaarVerified
                val resetPanVerified = resetPan.isNotBlank() && currentUser.panVerified
                val resetBankVerified = resetBank.isNotBlank() && currentUser.bankVerified

                database.userDao().updateUser(
                    currentUser.copy(
                        kycStatus = resetKycStatus,
                        aadhaarNumber = resetAadhaar,
                        aadhaarVerified = resetAadhaarVerified,
                        panNumber = resetPan,
                        panVerified = resetPanVerified,
                        bankAccountNumber = resetBank,
                        bankIfsc = resetIfsc,
                        bankVerified = resetBankVerified,
                        totalAgentEarnings = 0.0
                    )
                )
            }

            // 2. Clear demo DigiLocker session cache from preferences
            try {
                userRepository.clearDigiLockerSessionId()
            } catch (_: Exception) {}

            // 3. Delete all demo users from Room database (leaving ONLY real user accounts)
            database.userDao().deleteDemoUsers()

            // 4. Delete demo agent applications and staff admin requests
            ...
            Result.success("Demo data purged cleanly. Only real user data and genuine verifications remain.")
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
