...
async function sendTelegramMessage(chatId, text, replyMarkup = null) {
    try {
        ...
        const errDesc = (err.response?.data?.description || err.message || '').toLowerCase();
        console.error(`[Telegram send error for ${chatId}]:`, err.response?.data || err.message);

        // Fallback 1: Entity parse error -> retry with plain text
        if (errDesc.includes("can't parse entities") || errDesc.includes("tag") || errDesc.includes("parse")) {
            try {
                const plainPayload = {
                    chat_id: chatId,
        ...
        // Fallback 2: Invalid button URL -> retry without reply_markup
        if (errDesc.includes("button_url_invalid") || errDesc.includes("url")) {
            try {
                const noMarkupPayload = {
                    chat_id: chatId,
                    text: (text || '').length > 4000 ? (text || '').substring(0, 4000) : (text || ''),
                    parse_mode: 'HTML'
                };
                const retryRes = await axios.post(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, noMarkupPayload, { timeout: 8000 });
                return retryRes.data?.result || true;
            } catch (fallbackErr) {
                console.error(`[Telegram no-markup fallback error for ${chatId}]:`, fallbackErr.response?.data || fallbackErr.message);
            }
        }

        // Fallback 3: Plain text without markup
        try {
            const ultimatePayload = {
                chat_id: chatId,
                text: (text || '').replace(/<[^>]*>/g, '').substring(0, 4000)
            };
            const retryRes = await axios.post(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, ultimatePayload, { timeout: 8000 });
            return retryRes.data?.result || true;
        } catch (_finalErr) {}
...
app.post('/api/telegram/notify', async (req, res) => {
    ...
    } else if (url && (url.startsWith('http://') || url.startsWith('https://') || url.startsWith('tg://'))) {
        replyMarkup = {
